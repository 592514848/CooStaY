...
- (UITableView *)mainTableView{
    if(!_mainTableView){
        _mainTableView = [[UITableView alloc] initWithFrame: CGRectMake(0.0f, NAVIGATIONBAR_HEIGHT, SCREEN_WIDTH, self.contentHeight)];
...
-(void)updateRefreshFooterView{
    CGFloat height = MAX(self.mainTableView.contentSize.height, self.mainTableView.frame.size.height) + self.mainTableView.frame.origin.y;
    if (_refreshFooterView && [_refreshFooterView superview]){
        _refreshFooterView.frame = CGRectMake(0.0f, height, self.mainTableView.frame.size.width, SCREEN_HEIGHT);
    }
    else{
        _refreshFooterView = [[EGORefreshTableFooterView alloc] initWithFrame:
                              CGRectMake(0.0f, height, self.mainTableView.frame.size.width, SCREEN_HEIGHT)];
    }
