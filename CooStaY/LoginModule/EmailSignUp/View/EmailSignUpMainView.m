...
@implementation EmailSignUpMainView
...
- (BaseButton *)phoneRegisterButton{
    if(!_phoneRegisterButton){
        ...
        [_phoneRegisterButton addTarget: self action: @selector(phoneRegisterButtonClick:) forControlEvents: UIControlEventTouchUpInside];
    }
...
- (void)phoneRegisterButtonClick:(UIButton *)sender{
    if(IsNilOrNull(self.onPhoneRegisterClick)){
        return;
    }
    self.onPhoneRegisterClick();
}
@end
